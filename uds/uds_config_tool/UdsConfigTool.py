        ...
        for key, value in xmlElements.items():
            if value.tag == "DIAG-SERVICE":
                ...
                if serviceId == IsoServices.DiagnosticSessionControl:
                    ...
                    cls.rdbiContainer.add_checkSIDResponseFunction(
                        checkFunctions[0], humanName
                    )
                    cls.rdbiContainer.add_checkSIDLengthFunction(
                        checkFunctions[1], humanName
                    )
                    cls.rdbiContainer.add_checkDIDResponseFunction(
                        checkFunctions[2], humanName
                    )
                    cls.rdbiContainer.add_checkDIDLengthFunction(
                        checkFunctions[3], humanName
                    )
