...
from uds import Config, ResettableTimer, fillArray, iTp

# consts
# types
from uds.uds_communications.TransportProtocols.Lin.LinTpTypes import (
...
class LinTp(iTp):
    ...
    def __init__(self, configPath=None, **kwargs):
        ...
        self.__connection = LinBus.LinBus(
            19200
        )  # ... TODO: replace with something like the following
    ...
    def send(self, payload, functionalReq=False):  # TODO: functionalReq not used???
        ...
        while endOfMessage_flag is False:
            ...
            if state == LinTpState.SEND_SINGLE_FRAME:
                ...
                if stMinTimer.isExpired() and (self.__transmitBuffer is None):
                    txPdu[N_PCI_INDEX] += LinTpMessageType.CONSECUTIVE_FRAME << 4
                    txPdu[CONSECUTIVE_FRAME_SEQUENCE_NUMBER_INDEX] += sequenceNumber
                    txPdu[CONSECUTIVE_FRAME_SEQUENCE_DATA_START_INDEX:] = cfBlocks.pop(
                        0
                    )
                    self.transmit(txPdu)
