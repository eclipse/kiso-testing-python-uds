    ...
    def __call__(callback=None, filter=None, configPath=None, **kwargs):
        ...
        if connectionType == "virtual":
            ...
            if connectionName not in CanConnectionFactory.connections:
                CanConnectionFactory.connections[connectionName] = CanConnection(
                    callback,
                    filter,
            ...
            if channel not in CanConnectionFactory.connections:
                if CanConnectionFactory.bus:
                    CanConnectionFactory.connections[channel] = CanConnection(
                        callback, filter, CanConnectionFactory.bus, True
                    )
                else:
                    f_clock_mhz = int(
                        CanConnectionFactory.config["peak"]["f_clock_mhz"]
                    )
            ...
            if connectionKey not in CanConnectionFactory.connections:
                if CanConnectionFactory.bus:
                    CanConnectionFactory.connections[connectionKey] = CanConnection(
                        callback, filter, CanConnectionFactory.bus, True
                    )
                else:
                    serial = None
                    if "serial" in CanConnectionFactory.config["vector"]:
                        if (
                            str(CanConnectionFactory.config["vector"]["serial"]).upper()
                            == "AUTO"
                        ):
                            serial = CanConnectionFactory.detectVectorSerial()
                        else:
                            serial = int(
                                CanConnectionFactory.config["vector"]["serial"]
                            )
                    CanConnectionFactory.connections[connectionKey] = CanConnection(
                        callback,
                        filter,
